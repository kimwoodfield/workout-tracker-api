...
router.get("/", async (req, res) => {
  ...
  if (!req.session.userID) {
    ...
    logger.info(
      `Successfully retrieved the exercises for the current user. The user is logged in as userType: ${userType} from IP address: ${ip}`
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    let exercisesFound = await workout_trackerdb.doesExerciseExist(
      body,
      userID
    );
    ...
    if (exercisesFound.total) {
      logger.info(
        `User was unable to create an exercise because it already exists. The user is logged in as userType: ${userType} from IP address: ${ip}`
      );
    ...
    logger.info(
      `The user successfully added a new exercise! The user is logged in as userType: ${userType} from IP address: ${ip}`
    );
    ...
    logger.info(
      `The user was unsuccessful and unable to add a new exercise to the system. The user is logged in as userType: ${userType} from IP address: ${ip}`
    );
